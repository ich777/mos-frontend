  ...
  <template v-else-if="nutData && nutData.reachable">
    <v-row density="compact">
      ...
      <v-col cols="6" sm="6" md="4" xl="3" v-if="nutData?.data?.battery?.voltage !== undefined">
        ...
        <div class="text-body-2">{{ nutData.data.battery.voltage }}V</div>
      </v-col>
      <v-col cols="6" sm="6" md="4" xl="3" v-if="nutData?.data?.load !== undefined">
        <div class="text-caption text-medium-emphasis">
          <strong>{{ $t('load') }}</strong>
        </div>
        <div class="text-body-2">{{ nutData.data.load }}%</div>
      </v-col>
      <v-col cols="6" sm="6" md="4" xl="3" v-if="nutData?.data?.input?.voltage !== undefined">
        <div class="text-caption text-medium-emphasis">
          <strong>{{ $t('input voltage') }}</strong>
        </div>
        <div class="text-body-2">{{ nutData.data.input.voltage }}V</div>
      </v-col>
      <v-col cols="6" sm="6" md="4" xl="3" v-if="nutData?.data?.input?.frequency !== undefined">
        <div class="text-caption text-medium-emphasis">
          <strong>{{ $t('input frequency') }}</strong>
        </div>
        <div class="text-body-2">{{ nutData.data.input.frequency }}Hz</div>
      </v-col>
      <v-col cols="6" sm="6" md="4" xl="3" v-if="nutData?.data?.output?.voltage !== undefined">
        <div class="text-caption text-medium-emphasis">
          <strong>{{ $t('output voltage') }}</strong>
        </div>
        <div class="text-body-2">{{ nutData.data.output.voltage }}V</div>
      </v-col>
      <v-col cols="12" v-if="nutData?.data?.battery?.charge !== undefined">
        <div class="d-flex justify-space-between align-center w-100 mb-1">
          <strong><div class="text-caption text-medium-emphasis">{{ $t('battery charge') }}</div></strong>
        </div>
        <v-progress-linear
          :model-value="nutData.data.battery.charge"
          height="16"
          :color="nutData.data.battery.charge <= 20 ? 'error' : nutData.data.battery.charge <= 50 ? 'warning' : 'success'"
          rounded
        >
          <template #default>
            <span>
              <small>{{ nutData.data.battery.charge }}%</small>
            </span>
          </template>
        </v-progress-linear>
      </v-col>
